...
"""
    _class Rewriting_visitor(AstVisitor):
        ...
        def visit_arithmetic_unary(self, node, context):
            ...
            child = self.rewrite(node.value, context.get())
            if child != node.value:
        ...
        def visit_arithmetic_binary(self, node, context):
            ...
            left = self.rewrite(node.left, context.get())
            right = self.rewrite(node.right, context.get())
        ...
        def visit_at_time_zone(self, node, context):
            ...
            value = self.rewrite(node.value, context.get())
            Expression time_zone = rewrite(node.get_time_zone(), context.get())
        ...
        def visit_subscript_expression(self, node, context):
            ...
            base = self.rewrite(node.base, context.get())
            index = self.rewrite(node.index, context.get())
        ...
        def visit_between_predicate(self, node, context):
            ...
            value = self.rewrite(node.value, context.get())
            min = self.rewrite(node.min, context.get())
            max = self.rewrite(node.max, context.get())
        ...
        def visit_None_if_expression(self, node, context):
            ...
            first = self.rewrite(node.first, context.get())
            second = self.rewrite(node.second, context.get())
        ...
        def visit_if_expression(self, node, context):
            ...
            condition = self.rewrite(node.condition, context.get())
            Expression True_value = rewrite(node.get_true_value(), context.get())
            Expression False_value = None
            if node.get_false_value().is_present():
                False_value = rewrite(node.get_false_value().get(), context.get())
        ...
        def visit_simple_case_expression(self, node, context):
            ...
            operand = self.rewrite(node.operand, context.get())
        ...
        def visit_when_clause(self, node, context):
            ...
            operand = self.rewrite(node.operand, context.get())
            result = self.rewrite(node.result, context.get())
        ...
        def visit_try_expression(self, node, context):
            ...
            expression = self.rewrite(node.get_inner_expression(), context.get())
        ...
        def visit_function_call(self, node, context):
            ...
            if node.window.is_present():
                ...
                for expression in window.get_partition_by():
                    partition_by.add(rewrite(expression, context.get()))

                // Since Sort_item is not an Expression, but contains Expressions, just process the default rewrite inline with Function_call
                Immutable_list.Builder<Sort_item> order_by = Immutable_list.builder()
                for (Sort_item sort_item : window.get_order_by():
                    Expression sort_key = rewrite(sort_item.get_sort_key(), context.get())
                ...
                if rewritten_frame.is_present():
                    ...
                    if start.value.is_present():
                        value = self.rewrite(start.value.get(), context.get())
                    ...
                    if rewritten_end.is_present():
                        ...
                        if value.is_present():
                            Expression rewritten_value = rewrite(value.get(), context.get())
        ...
        def visit_lambda_expression(self, node, context):
            ...
            body = self.rewrite(node.body, context.get())
            if body != node.body:
        ...
        def visit_like_predicate(self, node, context):
            ...
            value = self.rewrite(node.value, context.get())
            pattern = self.rewrite(node.pattern, context.get())
            Expression escape = None
            if node.escape is not None:
                escape = rewrite(node.escape, context.get())
        ...
        def visit_in_predicate(self, node, context):
            ...
            value = self.rewrite(node.value, context.get())
            list = self.rewrite(node.get_value_list(), context.get())
        ...
        def visit_subquery_expression(self, node, context):
            ...
            // No default rewrite for Subquery_expression since we do not want to traverse subqueries
            return node
        ...
        def visit_extract(self, node, context):
            ...
            expression = self.rewrite(node.expression, context.get())
